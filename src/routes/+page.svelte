<script lang="ts">
  ...
  async function createUser(name: string) {
    let response = await invoke("create_user", { name });
    console.log(response);
  };

  async function getUsers() {
    let response = await invoke("get_users");
    names = response;
  };
...
<div class="container h-full mx-auto flex justify-center items-center">
  <div class="space-y-10 text-center">
    ...
    <!-- / -->
    <div class="space-y-2">
      <button class="btn variant-filled" on:click={testInvoke}>
        Generate Name
      </button>
      <h2>{name}</h2>
    </div>
    <div class="space-y-2">
      <button class="btn variant-filled" on:click={test}>
        Generate User
      </button>
      <h1>Names:</h1>
      {#each names as n}
